    ...
    <div className={`document-card ${isCollapsed ? 'collapsed' : ''}`} style={{ width: '100%', minWidth: '100%', boxSizing: 'border-box' }}>
      <div className="flex items-center justify-between p-6 bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200" style={{ width: '100%' }}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-bold text-gray-800 text-lg">{r.filename || `Document ${idx + 1}`}</h3>
            {avgConfText && (
              <span className={`confidence-badge mt-1 ${parseFloat(avgConfText) >= 85 ? 'confidence-high' : parseFloat(avgConfText) >= 70 ? 'confidence-medium' : 'confidence-low'}`}>{avgConfText} confidence</span>
            )}
          </div>
        </div>
        <button
      ...
      <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isCollapsed ? 'max-h-0 opacity-0' : 'max-h-[5000px] opacity-100'}`} style={{ width: '100%', boxSizing: 'border-box' }}>
        <div className="p-6" style={{ width: '100%', boxSizing: 'border-box' }}>
          ...
          {/* OCR text */}
          <div className="compare-ocr has-template">
            <div className="ocr-pane">
              <div className="ocr-col">
                ...
                <pre className="ocr-text" style={{whiteSpace:'pre-wrap', marginTop:8}}>{r.text}</pre>
                {r.qc_results && (
