...
test('titration without compliance metrics but DME issues -> verify_dme_prerequisites action', () => {
  const txt = 'Order: 95811 titration sleep study\nEquipment: CPAP user not tolerating pressure too high\nVendor: APRIA';
  const { result } = runExtraction(page(txt));
...
test('titration with compliance metrics does not add missing prereq flag', () => {
